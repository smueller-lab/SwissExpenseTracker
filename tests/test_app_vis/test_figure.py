...
def test_fig_income_expense_monthly_legend_shown_fixed_top() -> None:
    """This chart opts out of the site-wide right-side legend (see plots.md)."""
    df = _make_net_balance_month_df()
    fig = F.fig_IncomeExpenseMonthly(df)
    layout: Any = fig.layout
    assert layout.showlegend is True
    assert layout.legend.orientation == "h"
    assert layout.meta["legendFixed"] is True
