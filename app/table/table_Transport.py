...
s_transport_subcategory = ['Gas station', 'Parking', 'Train', 'Bus', 'Ferry', 'Car Dealership', 'Car sharing', 'Insurance']
...
pdf_transport = pdf_transport[(pdf_transport['category_main'].isin(s_transport_subcategory) | pdf_transport['category_second'].isin(s_transport_subcategory))].copy()

# remove car purchase from transport analytics
pdf_transport = pdf_transport[~((pdf_transport['Date'] == '2023-08-15') & (pdf_transport['category_transport'] == 'Car Dealership'))].copy()

# groupby category_transport and sum expenses
pdf_transport_sum = pdf_transport.groupby(['Year', 'category_transport'], as_index=False)['amount_CHF'].sum()

# %%
# Monthly Heatmap dataframe
s_transport_subcategory_heatmap = ['Gas station', 'Parking', 'Train', 'Bus', 'Ferry', 'Car sharing']
...
# remove car purchase from transport analytics
pdf_transport_heatmap = pdf_transport_heatmap[~((pdf_transport_heatmap['Date'] == '2023-08-15') & (pdf_transport_heatmap['category_transport'] == 'Car Dealership'))].copy()

# make heatmap
pdf_transport_heatmap = pdf_transport_heatmap.groupby(['Year', 'Month_num'], as_index=False)['amount_CHF'].sum()
pdf_transport_heatmap['Month_name'] = pd.to_datetime(pdf_transport_heatmap['Month_num'], format='%m').dt.month_name()
