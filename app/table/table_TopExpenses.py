...
# %%
pdf_TopExpenses = pdf[pdf["transaction_type"] == "expense"].sort_values(
    by="amount_CHF", ascending=False
)

# drop categories like Fried, Tax Services, and Finance
pdf_TopExpenses = pdf_TopExpenses[
    ~pdf_TopExpenses["category_main"].isin(
        [
            "Government",
            "Finance",
            "Friend",
...
pdf_TopExpenses.columns = [
    vis.vk_Variable_show[col] if col in vis.vk_Variable_show.keys() else col
    for col in pdf_TopExpenses.columns
]
