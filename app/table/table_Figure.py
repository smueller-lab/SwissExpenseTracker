...
pdf_Groceries = pdf_Groceries[
    pdf_Groceries["Merchant"].isin(vis.s_Merchant_Grocery)
].reset_index(drop=True)

pdf_Monthly = (
    pdf_Groceries.groupby(["MonthYear", "Merchant"], as_index=False)["amount_CHF"]
    .sum()
    .rename(columns={"amount_CHF": "total_CHF"})
)
pdf_Monthly["totalPeriod_CHF"] = pdf_Monthly.groupby("MonthYear")[
    "total_CHF"
].transform("sum")
pdf_Monthly["pct"] = pdf_Monthly["total_CHF"] / pdf_Monthly["totalPeriod_CHF"] * 100

pdf_Yearly = (
    pdf_Groceries.groupby(["Year", "Merchant"], as_index=False)["amount_CHF"]
    .sum()
    .rename(columns={"amount_CHF": "total_CHF"})
)
...
pdf_Monthly = (
    pdf_Food.groupby(["MonthYear", "category_second"], as_index=False)["amount_CHF"]
    .sum()
    .rename(columns={"amount_CHF": "total_CHF"})
)
pdf_Monthly["totalPeriod_CHF"] = pdf_Monthly.groupby("MonthYear")[
...
pdf_Yearly = (
    pdf_Food.groupby(["Year", "category_second"], as_index=False)["amount_CHF"]
    .sum()
    .rename(columns={"amount_CHF": "total_CHF"})
)
pdf_Yearly["totalPeriod_CHF"] = pdf_Yearly.groupby("Year")["total_CHF"].transform("sum")
...
pdf_Expenses = pdf[
    ...
    & (pdf["Merchant"] != "Viseca Payment Services SA")
].copy()
pdf_Expenses.loc[
    pdf_Expenses["Merchant"].str.contains("Revolut", case=False), "category_main"
] = "Revolut"
pdf_Expenses.loc[:, "Year"] = pdf_Expenses["Date"].dt.year

pdf_Summary = (
    pdf_Expenses.groupby(["category_main", "Year"])["amount_CHF"].sum().reset_index()
)
...
pdf_Summary["perc"] = pdf_Summary.groupby("Year")["amount_CHF"].transform(
...
pdf_Other = (
    pdf_Small.groupby("Year").agg({"amount_CHF": "sum", "perc": "sum"}).reset_index()
)
...
pdf_Summary = pdf_Summary.sort_values(
    ["Year", "category_main"], ascending=[True, True]
).reset_index(drop=True)
